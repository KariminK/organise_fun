...
class Group:
    ...
    def display_actions(self):
        while True:
            ...
            if answer == "\\h":
                ...
                with open(f"{self.group_dir}/lessons.of", "r+") as f:
                    self.delete_lesson(f)
            elif answer == "\\cp":
                ...
                parts = answer.split(" ")
                if len(parts) < 2:
                    self.print_gr("No command provided")
                    return
    ...
    def list_lessons(self, file):
        ...
        for lessonLine in file:
            lesson = lessonLine.strip().split(";")
            if len(lesson) < 4:
                continue
            name,created_at,ended_at,materials_number = lesson
            print(name.center(40, " "), created_at, ended_at, "", materials_number, sep="\t")
        print()
    ...
    def delete_lesson(self, file):
        lesson_name = self.input_gr("Enter lesson's name to delete: ")
        lesson_path = f"{self.group_dir}/lekcje/{lesson_name.replace(' ', '_')}"

        if not os.path.exists(lesson_path):
            self.print_gr("Lesson's directory doesn't exist!")
            return
        ...
        for lesson_line in lines:
            if not lesson_line.strip():
                continue

            parts = lesson_line.strip().split(";")
            if len(parts) != 4:
                continue

            name, created_at, ended_at, material_number = parts

            if name == lesson_name:
                deleted = True
                continue

            file.write(f"{name};{created_at};{ended_at};{material_number}\n")

        if deleted:
            shutil.rmtree(lesson_path)
            self.print_gr("Lesson deleted successfully!")
        else:
            self.print_gr("Lesson not found!")
